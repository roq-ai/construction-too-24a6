...
import { getRentalHistories, deleteRentalHistoryById } from 'apiSdk/rental-histories';
import { RentalHistoryInterface } from 'interfaces/rental-history';

type ColumnType = ColumnDef<RentalHistoryInterface, unknown>;

interface RentalHistoryListPageProps {
...
export function RentalHistoryListPage(props: RentalHistoryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RentalHistoryInterface>>(
    () => `/rental-histories?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteRentalHistoryById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: RentalHistoryInterface) => {
    if (hasAccess('rental_history', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/rental-histories/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/rental-histories/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('rental_history', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/rental-histories/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('rental_history', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="rental_history" />
        </Flex>

        {hasAccess('rental_history', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/rental-histories/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
