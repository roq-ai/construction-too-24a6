...
import { getRentalHistories } from 'apiSdk/rental-histories';
...
import { RentalHistoryListPage as ShowCaseList } from 'pages/rental-histories';

function DashboardPage() {
  const { session } = useSession();

  const rentalHistoryStat = useSWR<PaginatedInterface<RentalHistoryInterface>>(
    () => `/rental-histories`,
    () => getRentalHistories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: rentalHistoryStat?.data?.totalCount,
      isLoading: rentalHistoryStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Rental Histories
                </Text>
                <NextLink href={`rental-histories`} passHref legacyBehavior>
                  <Button
